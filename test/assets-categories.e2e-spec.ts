...
import { CreateAssetsCategoryDto } from '../src/assets-categories/dto/create-assets-category.dto';
import {
  AssetCategory,
  AssetCategorySchema,
} from '../src/assets-categories/assets-categories.schema';
const resource = 'AssetCategory';
...
const createData: CreateAssetsCategoryDto = {
  name: faker.name(),
...
afterAll(async () => {
  await assetCategory.deleteOne({ name: createData.name });
...
describe(`${resource} Detail`, () => {
  it('cannot get detail if not authenticated', async () => {
    found = await assetCategory.findOne({ name: createData.name });
